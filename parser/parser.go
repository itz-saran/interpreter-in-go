...
func (p *Parser) parseHashLiteral() ast.Expression {
	...
	for !p.peekTokenIs(token.RBRACE) {
		...
		if !p.advanceIfPeekMatches(token.COLON) {
			return nil
		}
		...
		if !p.peekTokenIs(token.RBRACE) && !p.advanceIfPeekMatches(token.COMMA) {
			return nil
		}
	}

	if !p.advanceIfPeekMatches(token.RBRACE) {
		return nil
	}
